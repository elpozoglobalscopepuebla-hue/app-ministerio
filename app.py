...
# =====================================================================
    ...
    query_est = f"""
        ...
        FROM conversaciones c JOIN estudiantes e ON c.estudiante_id = e.id
        WHERE strftime('%Y-%m', c.fecha) = '{mes_sel}' GROUP BY e.id ORDER BY 'Total General' DESC
    """
...
# =====================================================================
    ...
    if pwd == "elpozoregistradatos":
        ...
        with tab_activos:
            st.write("Doble clic para editar. Presiona Enter y luego Guardar.")
                ...
                if st.button("💾 Guardar Cambios Graduados"):
                    ...
                    for _, row in edited_grad.iterrows():
                        c.execute("""UPDATE estudiantes SET nombre=?, categoria=?, estado=?, telefono=?, escuela=?, edad=?, cumpleanos=? WHERE id=?""",
                                  (row['nombre'], row['categoria'], row['estado'], row['telefono'], row['escuela'], row['edad'], row['cumpleanos'], row['id']))
                    conn.commit(); st.success("Cambios guardados."); st.rerun()
            else: st.info("No hay graduados.")

        with tab_importar:
            st.info("Orden Excel: Nombre | Categoría | Teléfono | Escuela | Edad | Cumpleaños")
            datos_pegados = st.text_area("Pega los datos de Excel aquí:")
        ...
        with tab_borrar:
            ...
            if est_a_borrar:
                with st.popover("⚠️ Borrar Estudiantes Seleccionados"):
                    st.warning(f"Estás a punto de borrar **{len(est_a_borrar)} estudiante(s)** y TODO su historial de conversaciones. Esta acción es irreversible.")
